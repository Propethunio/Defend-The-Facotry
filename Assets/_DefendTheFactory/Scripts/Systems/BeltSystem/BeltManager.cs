...
public class BeltManager {
	...
	private List<BeltPath> beltPathList = new List<BeltPath>();
	...
	public bool AreBuildingsConnected(BaseBuildableObjectSO buildingOne, BaseBuildableObjectSO buildingTwo) {
		int pathsCount = beltPathList.Count;
		
		for (int index = 0; index < pathsCount; index++) {
			BeltPath beltPath = beltPathList[index];
			if (beltPath.beltList[0].parentBuilding == null || beltPath.beltList[^1].parentBuilding == null) continue;

			if (beltPath.beltList[0].parentBuilding.IsDataTheSame(buildingOne)) {
				if (beltPath.beltList[^1].parentBuilding.IsDataTheSame(buildingTwo)) {
					return true;
				}
			}
			else if (beltPath.beltList[0].parentBuilding.IsDataTheSame(buildingTwo)) {
				if (beltPath.beltList[^1].parentBuilding.IsDataTheSame(buildingOne)) {
					return true;
				}
			}
		}
