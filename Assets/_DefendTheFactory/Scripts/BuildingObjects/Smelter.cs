...
public class Smelter : PlacedObject, IItemStorage {
    ...
    public override string ToString() {
        ...
        foreach (ItemStack itemStack in inputItemStackList) {
            str += "I: " + itemStack.itemScriptableObject.itemName + "x" + itemStack.amount;
            str += "\n";
        }
        foreach (ItemStack itemStack in outputItemStackList) {
            str += "O: " + itemStack.itemScriptableObject.itemName + "x" + itemStack.amount;
            str += "\n";
        }
        return str;
    }
    ...
    public int GetItemStoredCount(ItemSO filterItemSO) {
        int amount = 0;
        foreach (ItemStack itemStack in outputItemStackList) {
            if (filterItemSO == GameAssets.i.itemSO_Refs.any || filterItemSO == itemStack.itemScriptableObject) {
    ...
    public bool TryGetStoredItem(ItemSO[] filterItemSO, out ItemSO itemSO) {
        ...
        if (ItemSO.IsItemSOInFilter(GameAssets.i.itemSO_Refs.any, filterItemSO) ||
            ...
            if (itemStack != null) {
                if (itemStack.amount > 0) {
                    itemStack.amount -= 1;
                    itemSO = itemStack.itemScriptableObject;
                    OnItemStorageCountChanged?.Invoke(this, EventArgs.Empty);
                    TriggerGridObjectChanged();
                    return true;
                } else {
    ...
    private void AddItemToInputItemStack(ItemSO itemSO, int amount = 1) {
        ...
        if (itemStack != null) {
            ...
            itemStack = new ItemStack { itemScriptableObject = itemSO, amount = amount };
    ...
    private ItemStack GetInputItemStackWithItemType(ItemSO itemSO) {
        foreach (ItemStack itemStack in inputItemStackList) {
            if (itemStack.itemScriptableObject == itemSO) {
                return itemStack;
            }
        }
        return null;
